...
public class Main {
    public static void main(String[] args) {
        ...
        Aircraft freighter = new Freighter("Boeing777", "777F", 301, 103000,37);
        ...
        Flight flight4 = new Flight("FL" + drone.getTailNumber(), LocalTime.of(11, 30), LocalTime.of(12, 0));

        // Create airport with runways
        ArrayList<String> runways = new ArrayList<>(Arrays.asList("Runway1"));
        Airport airport = new Airport("International Airport", runways);

        // Create and solve scheduling problem
        SchedulingProblem problem = new SchedulingProblem(airport, Arrays.asList(flight1, flight2, flight3, flight4));

        try {
            problem.solve();
            Map<Flight, String> solution = problem.getSolution();
            System.out.println("Addtional Runways: " + problem.getAdditionalRunwaysNeeded());
            // Print solution
